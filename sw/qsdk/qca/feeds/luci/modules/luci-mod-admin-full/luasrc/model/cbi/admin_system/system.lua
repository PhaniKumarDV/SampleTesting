...
o = s:taboption("logging", Flag, "templogstatus", "Enable Temperature Log")
o.rmempty = false
o.datatype    = "bool"
function o.write(self, section, value)
	sys.call("env -i /etc/init.d/temp-log restart >/dev/null")
    Flag.write(self, section, value)
end



o = s:taboption("logging", Value, "temploginterval", "Temperature Log Interval")
o.optional    = true
o.placeholder = 60
o.datatype    = "uinteger"
...
-- GPS
--
s = m:section(TypedSection, "gps", translate("GPS"))
s.anonymous = true
s.addremove = false
o = s:option(Flag, "status", "Enable GPS")
o.rmempty = false
o.datatype    = "bool"
function o.write(self, section, value)
	if value == "1" then
		sys.init.enable("gps")
		sys.call("env -i /etc/init.d/gps start >/dev/null")
	else
		sys.call("env -i /etc/init.d/gps stop >/dev/null")
		sys.init.disable("gps")
	end

    Flag.write(self, section, value)
end

o = s:option(Value, "interval", translate("Interval"), "sec")
o = s:option(DummyValue, "latitude", translate("Latitude"))
o = s:option(DummyValue, "longitude", translate("Longitude"))
o = s:option(DummyValue, "altitude", translate("Altitude"))

--
-- LED
--
s = m:section(TypedSection, "led", translate("LED Configuration"))
s.anonymous = true
s.addremove = false
o = s:option(Flag, "status", "Enable LEDs")
o.rmempty = false
o.datatype    = "bool"
function o.write(self, section, value)
	if value == "1" then
		sys.init.enable("leds")
		sys.call("env -i /etc/init.d/leds start >/dev/null")
	else
		sys.call("env -i /etc/init.d/leds stop >/dev/null")
		sys.init.disable("leds")
	end
...
local is_bit_set = 0;
s = m:section(TypedSection, "customer", translate("Customer Configuration"))
s.anonymous = true
s.addremove = false
o = s:option(Value, "name", translate("Customer Name"))
function o.write(self, section, value)
	if ( is_bit_set == 0 ) then
...
if (string.match(mode,"sta") and string.match(wds,"1") ) then
	o = s:option(Value, "linkid", translate("Link ID"))
	function o.write(self, section, value)
		if ( is_bit_set == 0 ) then
			sys.call("/etc/init.d/network restart >/dev/null")
			is_bit_set = 1;
