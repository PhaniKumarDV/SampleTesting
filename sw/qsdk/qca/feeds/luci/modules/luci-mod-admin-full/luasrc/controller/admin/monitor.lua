...
function action_wireless1()
    local res = luci.util.exec("athstats -i wifi1 | grep tx_packets | sed 's/ast_tx_packets://'")
    local txpkt = string.gsub(res, "\n", "")
    res = luci.util.exec("athstats -i wifi1 | grep rx_packets | sed 's/ast_rx_packets://'")
...
function action_wireless2()
    local res = luci.util.exec("athstats -i wifi0 | grep tx_packets | sed 's/ast_tx_packets://'")
    local txpkt = string.gsub(res, "\n", "")
    res = luci.util.exec("athstats -i wifi0 | grep rx_packets | sed 's/ast_rx_packets://'")
    local rxpkt = string.gsub(res, "\n", "")
