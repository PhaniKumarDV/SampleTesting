...
m:chain("network")
m:chain("firewall")
m.redirect = luci.dispatcher.build_url("admin/wireless/wireless")
...
function m.parse(map)
	if m:formvalue("cbid.wireless.%s.__toggle" % wdev:name()) then
		...
		luci.http.redirect(luci.dispatcher.build_url("admin/wireless/wireless", arg[1]))
		return
	end
	Map.parse(map)
end
...
st.template = "admin_wireless/wifi_status"
st.ifname   = arg[1]
...
if found_sta then
	...
	function ch.formvalue(self, section)
		...
		local tmpMode = nil
		local realMode = nil
		if m:formvalue(self:cbid(section) .. ".band") then
			tmpBand = m:formvalue(self:cbid(section) .. ".band")
		end
		if m:formvalue(self:cbid(section) .. ".mode") then
			tmpMode = m:formvalue(self:cbid(section) .. ".mode")
		end
		--luci.sys.call("env -i echo 'Amigo debug ====> tmpBand %q' > /dev/console" % tmpBand)
		--luci.sys.call("env -i echo 'Amigo debug ====> tmpMode %q' > /dev/console" % tmpMode)
		if tmpMode == "n" and tmpBand == "11g" then
			realMode = "11ng"
		elseif tmpMode == "n" and tmpBand == "11a" then
			realMode = "11na"
		elseif tmpMode == "ac" and tmpBand == "11a" then
			realMode = "11ac"
		else
			realMode = nil
		end
		--luci.sys.call("env -i echo 'Amigo debug ====> realMode %q' > /dev/console" % realMode)
		return {
			(hw_modes.ad and "11ad") or realMode or (hw_modes.g and "11g" or "11a") or "auto",
	...
	if (radio == "11ac" or radio == "11na" or radio == "11a") then 
		s:taboption("general", Value, "distance", translate("Distance"), "Km")
		rate = s:taboption("general", ListValue, "rate", translate("Data Rate"))
		rate:value("auto", translate("Auto"))
		rate:value("0", translate("MCS0"))
		rate:value("1", translate("MCS1"))
		rate:value("2", translate("MCS2"))
		rate:value("3", translate("MCS3"))
		rate:value("4", translate("MCS4"))
		rate:value("5", translate("MCS5"))
		rate:value("6", translate("MCS6"))
		rate:value("7", translate("MCS7"))
		if (radio == "11ac" or radio == "11na") then 
			rate:value("8", translate("MCS8"))
			rate:value("9", translate("MCS9"))
			rate:value("10", translate("MCS10"))
			rate:value("11", translate("MCS11"))
			rate:value("12", translate("MCS12"))
			rate:value("13", translate("MCS13"))
			rate:value("14", translate("MCS14"))
			rate:value("15", translate("MCS15"))
			if (radio == "11ac") then 
				rate:value("16", translate("MCS16"))
				rate:value("17", translate("MCS17"))
				rate:value("18", translate("MCS18"))
				rate:value("19", translate("MCS19"))
			end
		end
	end
...
if (radio == "11ng" or radio == "11g") then 
...
if hwtype == "qcawifi" then
	...
	mp:value("allow", translate("Allow MAC List"))
	mp:value("deny", translate("Deny MAC List"))
 ...
 -------------------- qcawifi Interface ----------------------
if hwtype == "qcawifi" then
	mode:value("ap-wds", translate("Outdoor Base"))
	mode:value("sta-wds", translate("Outdoor Subscriber"))
